...
`r length(de$a.icu)` patients were admitted into ICU. The observed mean duration from hospital admission to ICU is `r round(mean(de$a.icu, na.rm=T), 1)` days (SD: `r round(sd(de$a.icu, na.rm=T), 1)` days). 
...
`r de$p.o2`% of patients received some degree of oxygen supplementation.  Of these, `r de$p.NIV`% received non-invasive ventilation and `r de$p.In.Ven`% received invasive mechanical ventilation.
